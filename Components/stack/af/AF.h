...
/*********************************************************************
 * ½ÚµãµØÖ·Ä£Ê½(Endpoint  Descriptions)
 */
/**
 * AddrBroadcast¹ã²¥:
 *	afAddrType_t.addr.shortAddr = NWK_BROADCAST_SHORTADDR_DEVZCZR			///< 0xFFFC: ËùÓÐÂ·ÓÉÆ÷ºÍÐ­µ÷Æ÷(Routers and Coordinators)
 *	afAddrType_t.addr.shortAddr = NWK_BROADCAST_SHORTADDR_DEVRXON			///< 0xFFFD: ËùÓÐ´¦ÓÚIDLE×´Ì¬ÏÂµÄÉè±¸(Everyone with RxOnWhenIdle == TRUE)
 *	afAddrType_t.addr.shortAddr = NWK_BROADCAST_SHORTADDR_DEVALL			///< 0xFFFF: ¹ã²¥,·¢ËÍµ½ËùÓÐÉè±¸(°üÀ¨ÐÝÃßµÄ),¶ÔÓÚÐÝÃßµÄ,ÐÅÏ¢±»±£´æÔÚËüµÄ¸¸½Úµã,Ö±µ½ÐÝÃß½áÊø½ÓÊÕ/timeout(ÓÉf8wConfig.cfgÖÐNWK_INDIRECT_MSG_TIMEOUTÖ¸¶¨)
 */
typedef enum
{
  afAddrNotPresent = AddrNotPresent,	///< ¼ä½ÓÑ°Ö·,ÓÃÒ»¸ö´æ´¢ÔÚ·¢ËÍÉè±¸Ð­ÒéÕ»µÄ"°ó¶¨±í¸ñ"´úÌæÄ¿µÄµØÖ·,±í¸ñÖÐÓÐ±»°ó¶¨µÄÉè±¸µØÖ·
  afAddr16Bit      = Addr16Bit,			///< µãµ½µã´«Êä
  afAddr64Bit      = Addr64Bit,
  afAddrGroup      = AddrGroup,			///< ·¢µ½Ò»¸öÊý¾Ý×é
  afAddrBroadcast  = AddrBroadcast		///< ¹ã²¥´«Êä
} afAddrMode_t;
...
typedef struct
{
  union
  {
    uint16      shortAddr;		///< 16Î»ÍøÂçµØÖ·
    ZLongAddr_t extAddr;		///< 64Î»À©Õ¹µØÖ·(MACµØÖ·)
  } addr;
